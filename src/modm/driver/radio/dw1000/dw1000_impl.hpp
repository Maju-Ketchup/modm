...
//--------------------------------------------------------------modm-Functions-Puplic------------------------------------------------------------------------



template < typename Spi, typename Cs, typename Reset, typename Irq >
uint8_t
modm::Dw1000< Spi, Cs, Reset, Irq >::frame_seq_nb = 0;

template < typename Spi, typename Cs, typename Reset, typename Irq >
uint64_t
modm::Dw1000< Spi, Cs, Reset, Irq >::hostaddress = 0xFFFFFFFFFFFFFFFF;

template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::TX_ANT_DLY = 16436;

template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::RX_ANT_DLY = 16436;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
modm::dw1000::IRQreason
modm::Dw1000< Spi, Cs, Reset, Irq >::getIRQReason()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readStatusMRegister()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::setIRQ(uint32_t registermask)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint64_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readSystemTimestamp64()
{
	uint8_t ts_tab[5];
	uint64_t ts = 0;
	readsystime(ts_tab);
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint64_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readTXTimestamp64()
{
	uint8_t ts_tab[5];
	uint64_t ts = 0;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint64_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readRXTimestamp64()
{
	uint8_t ts_tab[5];
	uint64_t ts = 0;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::enableExternalSync()
{
	write32bitreg(EXT_SYNC_ID,EC_CTRL_OSTRM| 33 << 3);
}
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::setCrystaltrim(uint8_t trim)
{
	if (trim < 0b100000){
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
bool
modm::Dw1000< Spi, Cs, Reset, Irq >::sendAt(int size, uint8_t data[], uint32_t time)
{
	...
	{
		//MODM_LOG_DEBUG << "DATA WAS SENT DELAYED" << modm::endl;
		return true;
	}
	else
	{
		MODM_LOG_ERROR << "DWM1000: Could not send because delayed time < systemtime" << modm::endl;
		MODM_LOG_DEBUG << int (readsystimestamphi32() - time) << modm::endl;
		return false;
	}
}
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
bool
modm::Dw1000< Spi, Cs, Reset, Irq >::send(int size, uint8_t data[])
{
	if (size < 3 || size > 1023)
	{return false;}
	write32bitreg(SYS_STATUS_ID, SYS_STATUS_TXFRS);
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::readrx(uint32_t size, uint8_t buffer[])
{
	if (read32bitreg(SYS_STATUS_ID & SYS_STATUS_RXFCG)) {
		write32bitreg(SYS_STATUS_ID, SYS_STATUS_RXFCG);
		readrxdata(buffer, size, 0);
	} else {
		MODM_LOG_ERROR << "!!!DWM1000: ERROR NO FRAME TO READ!!!" << modm::endl;
	}
}

template < typename Spi, typename Cs, typename Reset, typename Irq >
uint16_t
modm::Dw1000< Spi, Cs, Reset, Irq >::rxlength()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::displayMaskRegister ()
{
	uint32_t maskregister;
	maskregister = read32bitreg(SYS_MASK_ID);
	MODM_LOG_DEBUG.printf ("DWM1000 StatusMaskregister: %lx \n",maskregister);
}

template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::displayStatusRegister ()
{
	uint32_t statusregister;
	statusregister = read32bitreg(SYS_STATUS_ID);
	MODM_LOG_DEBUG.printf ("DWM1000 Statusregister: %lx \n", statusregister);
}

template < typename Spi, typename Cs, typename Reset, typename Irq >
bool
modm::Dw1000< Spi, Cs, Reset, Irq >::isFrameSent()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::trxdisable()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
bool
modm::Dw1000< Spi, Cs, Reset, Irq >::checkForRXError()
{
	uint32_t status_reg = read32bitreg(SYS_STATUS_ID);
	if (status_reg & SYS_STATUS_ALL_RX_TO)
	{MODM_LOG_DEBUG << "!!!DWM1000: RECEIVE TIME OUT!!!" << modm::endl;}
	if (status_reg & SYS_STATUS_ALL_RX_ERR)
	{MODM_LOG_DEBUG << "!!!DWM1000: RECEIVE ERROR OCCURRED!!!" << modm::endl;}
	return (read32bitreg(SYS_STATUS_ID) & SYS_STATUS_ALL_RX_ERR);
}

template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::rxdisable()
{
	trxdisable();
	rxreset();
}
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
modm::dw1000::local_data_t
modm::Dw1000< Spi, Cs, Reset, Irq >::dw1000local;

template < typename Spi, typename Cs, typename Reset, typename Irq >
int
modm::Dw1000< Spi, Cs, Reset, Irq >::writetospi(
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readdevid()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::setxtaltrim(uint8_t value)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::_disablesequencing()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::_aonarrayupload()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::softreset()
{
	_disablesequencing();
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::_loaducodefromrom()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::_otpread(uint32_t address)
{
	uint32_t ret_data;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::_enableclocks(int clocks)
{
	uint8_t reg[2];
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::_configlde(int prfIndex)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::configure(config_t *conf)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
int
modm::Dw1000< Spi, Cs, Reset, Irq >::initialise(uint16_t conf)
{
	uint16_t otp_addr = 0;
	uint32_t ldo_tune = 0;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
int
modm::Dw1000< Spi, Cs, Reset, Irq >::writetxdata(uint16_t txFrameLength, uint8_t *txFrameBytes, uint16_t txBufferOffset)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::writetxfctrl(uint16_t txFrameLength, uint16_t txBufferOffset, int ranging)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::write8bitoffsetreg(int regFileID, int regOffset, uint8_t regval)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::rxEnable()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
bool
modm::Dw1000< Spi, Cs, Reset, Irq >::rxEnableAt(uint32_t time)
{
	setdelayedtrxtime(time);
	int result;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
int
modm::Dw1000< Spi, Cs, Reset, Irq >::rxenable(RX_Mode mode)
{
	uint16_t temp ;
	uint8_t temp1 ;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::readrxdata(uint8_t *buffer, uint16_t length, uint16_t rxBufferOffset)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::read32bitreg(uint16_t addr)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint16_t
modm::Dw1000< Spi, Cs, Reset, Irq >::read16bitoffsetreg(int regFileID,int regOffset)
{
	uint16_t  regval = 0 ;
	uint8_t   buffer[2] ;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::read32bitoffsetreg(int regFileID, int regOffset)
{
	uint32_t  regval = 0 ;
	int     j ;
	uint8_t   buffer[4] ;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::writetodevice(uint16_t recordNumber, uint16_t index, uint32_t length , const uint8_t *buffer)
{
	uint8_t header[3];
	int cnt = 0;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::write16bitoffsetreg(int regFileID,int regOffset,uint16_t regval)
{
	uint8_t buffer[2];
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::write32bitoffsetreg(int regFileID, int regOffset, uint32_t regval)
{
	int j;
	uint8_t buffer[4];
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::write32bitreg(int x, uint32_t y)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::readfromdevice(uint16_t  recordNumber, uint16_t  index, uint32_t  length, uint8_t buffer[])
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
bool
modm::Dw1000< Spi, Cs, Reset, Irq >::init(config_t &conf){
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readTXTimestamplo32(){
	return read32bitreg(TX_TIME_ID);
}

template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readTXTimestamphi32(){
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint8_t
modm::Dw1000< Spi, Cs, Reset, Irq >::read8bitoffsetreg(int regFileID, int regOffset){
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::readsystime(uint8_t timestamp[])
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readsystimestamphi32(void)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readsystimestamplo32(void)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readRXTimestamplo32(void)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint32_t
modm::Dw1000< Spi, Cs, Reset, Irq >::readRXTimestamphi32(void)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::setTXAntennaDelay(uint16_t txDelay)
{
	TX_ANT_DLY = txDelay;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::setRXAntennaDelay(uint16_t rxDelay)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::setrxaftertxdelay(uint32_t rxDelayTime)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::setrxtimeout(uint16_t time)
{
	uint8_t temp ;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::rxreset(void)
{
	// Set RX reset
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::readRXTimestamp(uint8_t timestamp[])
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::readTXTimestamp(uint8_t timestamp[])
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::setdelayedtrxtime(uint32_t starttime)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::syncrxbufptrs(void)
{
	uint8_t  buff ;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::forcetrxoff(void)
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
uint64_t
modm::Dw1000< Spi, Cs, Reset, Irq >::get_rx_timestamp_u64(void)
{
	uint8_t ts_tab[5];
	uint64_t ts = 0;
	int i;
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
modm::dw1000::decaIrqStatus_t
modm::Dw1000< Spi, Cs, Reset, Irq >::decamutexon()
{
...
template < typename Spi, typename Cs, typename Reset, typename Irq >
void
modm::Dw1000< Spi, Cs, Reset, Irq >::decamutexoff(decaIrqStatus_t s)
{
	if (s == 0) {
